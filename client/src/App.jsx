...
const App = () => {
  ...
  useEffect(() => {
      ...
      const loadApodData = async (startDate) => {
          ...
          const newApodData = await Promise.all(dates.map(date => fetchApodData(date)));
          setApodData(apodData => [...apodData, ...newApodData.filter(item => item)]);
      };
      ...
      <>
          ...
          <div className="gallery">
              {apodData.map((apod, index) => (
                  <div key={index} className="item" onClick={() => openPopup(apod)}>
